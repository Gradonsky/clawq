...
import { toHex, keccak256, encodePacked, defineChain } from "viem";
...
function formatPrice(cmd) {
    if (!cmd.pricePerUnit || cmd.pricePerUnit === 0)
        return "FREE";
...
async function cmdAgents() {
    ...
    if (searchTerm) {
        agents = agents.filter(a => a.agent_id.toLowerCase().includes(searchTerm) ||
            a.agent_name.toLowerCase().includes(searchTerm) ||
            (a.description || "").toLowerCase().includes(searchTerm));
    }
    if (onlyOnline)
        agents = agents.filter(a => a.is_online);
    if (onlyFree) {
    ...
    console.log(pad("AGENT ID", col.id) + pad("NAME", col.name) + pad("STATUS", col.status) +
    ...
    for (const agent of agents) {
        ...
        if (maxP === 0)
            ...
            priceRange = `FREE-$${maxP}`;
        else if (minP === maxP)
            priceRange = `$${minP}`;
        else
            priceRange = `$${minP}-$${maxP}`;
        const status = agent.is_online ? "\x1b[32mON\x1b[0m    " : "\x1b[31mOFF\x1b[0m   ";
        console.log(pad(agent.agent_id, col.id) + pad(agent.agent_name, col.name) + status +
...
async function cmdInfo() {
    ...
    if (!agent) {
        const similar = agents.filter(a => a.agent_id.includes(agentId) || a.agent_name.toLowerCase().includes(agentId.toLowerCase()));
        if (JSON_FLAG) {
    ...
    if (agent.nft_id)
        console.log(`  NFT Token:   #${agent.nft_id}`);
    if (agent.creator)
...
async function cmdQuery() {
    ...
    const timeout = setTimeout(() => {
        if (!responseReceived) {
            if (JSON_FLAG)
                console.log(JSON.stringify({ error: "timeout", message: "No response after 60s" }));
            else
    ...
    ws.on("open", () => {
        if (!JSON_FLAG)
            console.log("[1/4] Connected. Requesting challenge...");
    ...
    ws.on("message", async (raw) => {
        ...
        if (msg.type === "challenge") {
            ...
            if (!JSON_FLAG)
                console.log("[2/4] Challenge received. Signing...");
            ws.send(JSON.stringify({
                type: "auth",
        ...
        if (msg.type === "auth" || msg.type === "authenticated" || msg.type === "success") {
            ...
            if (privateRooms.length > 0)
                privateRoomId = privateRooms[0].id;
            if (!privateRoomId) {
                const roomObjs = msg.data?.room_objects || [];
                const priv = roomObjs.find((r) => !r.is_public);
                if (priv)
            ...
            if (!privateRoomId) {
                ...
                if (rooms.length > 0)
                    privateRoomId = typeof rooms[0] === "string" ? rooms[0] : rooms[0].id;
            }
            if (!privateRoomId) {
                if (JSON_FLAG)
                    console.log(JSON.stringify({ error: "no_room", message: "No room available" }));
                else
                    console.error("  No room available. Cannot send query.");
                done(1);
                return;
            }
            if (!JSON_FLAG) {
        ...
        if (msg.type === "agent_selected") {
            ...
            if (!JSON_FLAG)
                console.log(`  Coordinator → ${name}${reason ? ` (${reason})` : ""}`);
        }
        // ─── Agent response ───────────────────────────────────────
        ...
        if (msg.type === "task_quote") {
            ...
            if (isFree) {
                ...
                try {
                    ...
                    if (!JSON_FLAG)
                        console.log("  Payment signed. Confirming task...");
                    ws.send(JSON.stringify({
                        type: "confirm_task",
                        data: { task_id: q.task_id },
                        room: privateRoomId,
        ...
        if (msg.type === "error") {
            ...
            if (JSON_FLAG)
                console.log(JSON.stringify({ type: "error", code, message: errMsg }));
            else
        ...
        if (msg.type === "rate_limit_notification") {
            if (JSON_FLAG)
                console.log(JSON.stringify({ type: "rate_limit", message: msg.content || msg.data?.message }));
            else
                console.log(`  RATE LIMITED: ${msg.content || msg.data?.message || "Too many requests"}`);
        }
    });
    ws.on("error", (err) => {
        if (JSON_FLAG)
...
async function fetchNetworkConfig(networkName) {
    if (!networkCache) {
        const res = await fetch(`${BACKEND_URL}/api/networks`);
        if (!res.ok)
    ...
    if (networkCache[networkName])
        return networkCache[networkName];
    for (const [, cfg] of Object.entries(networkCache)) {
        if (cfg.caip2 === networkName || cfg.caip2 === `eip155:${networkName}`)
            return cfg;
    }
    // Default to peaq
    if (networkCache["peaq"])
        return networkCache["peaq"];
...
async function signPayment(privateKey, quoteData) {
    ...
    const salt = (quoteData.salt || toHex(crypto.getRandomValues(new Uint8Array(32))));
    const facilitatorFee = quoteData.facilitator_fee || "100";
    const hook = (quoteData.hook || network.transferHook);
    const hookData = (quoteData.hook_data || "0x");
    const totalValueStr = (BigInt(amountStr) + BigInt(facilitatorFee)).toString();
    const now = Math.floor(Date.now() / 1000);
    const validAfter = now - 60;
    const validBefore = now + 60;
    const commitment = keccak256(encodePacked(["string", "uint256", "address", "address", "address", "uint256", "uint256", "uint256", "bytes32", "address", "uint256", "address", "bytes32"], [
        "X402/settle/v1",
        BigInt(chain.id),
        settlementRouter,
        asset,
